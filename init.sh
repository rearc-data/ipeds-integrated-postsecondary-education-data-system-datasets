...
# creating a layer and deploy zip packages, these commands may need to be adjusted depending on folder structure and dependencies
rm -f dependencies.zip
mkdir dependencies dependencies/python
cp -r bin dependencies/.
cd dependencies/bin; unzip -u ../../chromium.zip 
pip3 install -r requirements.txt -t dependencies/python
cd dependencies; zip -9qr dependencies.zip .
cp dependencies/dependencies.zip .
rm -rf dependencies

rm -f preprocessing.zip
mkdir preprocessing 
cp -r src preprocessing/.
cd deploy; zip -9qr preprocessing.zip .
cp preprocessing/preprocessing.zip .
rm -rf preprocessing

echo "uploading dependencies.zip and preprocessing.zip to s3"
aws s3 cp layer.zip s3://$S3_BUCKET/$DATASET_NAME/automation/SeleniumChromiumLayer.zip --region "$REGION" "$PROFILE"
aws s3 cp preprocessing.zip s3://$S3_BUCKET/$DATASET_NAME/automation/pre-processing-code.zip --region "$REGION" "$PROFILE"
...
# zip -r pre-processing-code.zip . -x "*.whl" -x "*.dist-info/*" -x "bin/*" -x "**/__pycache__/*") 
...
aws cloudformation create-stack --stack-name "$CFN_STACK_NAME" --template-body file://cfn.yaml --parameters ParameterKey=S3Bucket,ParameterValue="$S3_BUCKET" ParameterKey=DataSetName,ParameterValue="$DATASET_NAME" ParameterKey=DataSetArn,ParameterValue="$DATASET_ARN" ParameterKey=ProductId,ParameterValue="$PRODUCT_ID" ParameterKey=Region,ParameterValue="$REGION" --region "$REGION" --capabilities "CAPABILITY_AUTO_EXPAND" "CAPABILITY_NAMED_IAM" "CAPABILITY_IAM" "$PROFILE"
